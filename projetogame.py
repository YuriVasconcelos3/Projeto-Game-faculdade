...
def main():
    ...
    for i in range(3):
        imagens.append(pg.image.load('Assets/Textures/variaveis'+str(i+1)+'.jpg').convert())
    for i in range(5):
        imagens2.append(pg.image.load('Assets/Textures/level'+str(i+1)+'.jpg').convert())
    for i in range(8):
        imagens3.append(pg.image.load('Assets/Textures/lista'+str(i+1)+'.jpg').convert())
    for i in range(9):
        imagens4.append(pg.image.load('Assets/Textures/pilhas'+str(i+1)+'.jpg').convert())
    for i in range(7):
        imagens5.append(pg.image.load('Assets/Textures/final'+str(i+1)+'.jpg').convert())
    

    msg = "Pressione uma tecla..."
    splash[0].copy()
    surf = splash[0].copy()
    splash_screen(msg, splash[0], clock, font, screen)
    msg = " "
    level_images = {
    1: "caminho/para/imagem/nivel1.png",
    2: "caminho/para/imagem/nivel2.png",
    3: "caminho/para/imagem/nivel3.png",
    4: "caminho/para/imagem/nivel4.png",
    5: "caminho/para/imagem/nivel5.png"
}
    while running:
        pg.display.update()
        ...
        if not pause and (player_health <= 0 or (exit2 == 0  and int(posx) == exitx and int(posy) == exity)):
            ...
            if player_health <= 0:
                ...
                if level == 2:
                    msg = "C"
                    for i in range(5):
                        surf = imagens2[i].copy()
                        splash_screen(msg, surf, clock, font, screen)
                        pause, clickdelay = 1, ticks
                if level == 3:
                    msg = "C"
                    for i in range(8):
                        surf = imagens3[i].copy()
                        splash_screen(msg, surf, clock, font, screen)
                        pause, clickdelay = 1, ticks
                if level == 4:
                    msg = "C"
                    for i in range(9):
                        surf = imagens4[i].copy()
                        splash_screen(msg, surf, clock, font, screen)
                        pause, clickdelay = 1, ticks
                if level == 5:
                    msg = "C"
                    for i in range(7):
                        surf = imagens5[i].copy()
                        splash_screen(msg, surf, clock, font, screen)
                        pause, clickdelay = 1, ticks
                surf.blit(surf, (0, 0))
            pause, clickdelay = 1, ticks
        ...
        if  pause:
            ...
            if newgame == 1:
                ...
                if story:
                    posx, posy, rot, rotv, maph, mapp, exitx, exity, stepscount, size = load_map(level)
                    nlevel = level_textures[level]
                    
                else:
                    size = np.random.randint(10+level*2, 16+level*2)
                    nenemies = size #number of enemies
                    posx, posy, rot, rotv, maph, mapp, exitx, exity, stepscount = gen_map(size)
                    nlevel = [np.random.randint(0,3), #sky1
                            np.random.randint(0,5), #floorwall
                ...
                livros, spsize, sword, swordsp = get_sprites(nlevel[5])
            ...
            frame = new_frame(posx-0.2*np.cos(rot), posy-0.2*np.sin(rot), rot, frame, sky, floor, hres, halfvres,
                              mod, maph, size, wall, mapp, exitx, exity, nenemies, rotv, door, window, bwall, exit2)
            ...
            surf, en = draw_sprites(surf, livros, enemies, spsize, hres, halfvres, ticks, sword, swordsp, rotv)
            ...
            pg.display.set_caption(" livros restantes: " + str(nenemies) + " FPS: " + str(fps)+ ' ')
            posx, posy, rot, rotv = movimento(pg.key.get_pressed(), posx, posy, rot, maph, er, rotv)


def movimento(pressed_keys, posx, posy, rot, maph, et, rotv):
...
def gen_map(size):
    ...
    return posx, posy, rot, rotv, maph, mapp, exitx, exity, stepscount

def load_map(level):
    mapp = pg.surfarray.array3d(pg.image.load('Assets/Levels/map'+str(level)+'.png'))
    size = len(mapp)
    ...
    return posx, posy, rot, rotv, maph, mapp/255, exitx, exity, stepscount, size

@njit(cache=True)
def new_frame(posx, posy, rot, frame, sky, floor, hres, halfvres, mod, maph, size, wall, mapp,
    ...
    for i in range(hres):
        ...
        if n2 > 0.5 and 3*h > int(halfvres/(n*cos2 + 0.000001)): #draw upper wall
            ...
            if x%1 < 0.01 or x%1 > 0.99:
                xx = int(x*1%1*1199)
                xxx = y%1
            yy= np.linspace(0, 3, int(h*2))*205%600
        ...
        if x%1 < 0.01 or x%1 > 0.99:
            xx = int(x*1%1*1199)
            xxx = y%1
        yy= np.linspace(0, 3, h*2)*205%600
        ...
        #c = mapp[int(x)%(size-1)][int(y)%(size-1)]
        for k in range(h*2):
            if halfvres - h +k +offset >= 0 and halfvres - h +k +offset < 2*halfvres:
                if ash and 1-k/(2*h) < 1-xx/99:
                    shade, ash = 0.6*shade, 0
                frame[i][halfvres - h +k+offset ] = shade*wall[xx][int(yy[k])]
            if halfvres+3*h-k+offset-1 < halfvres*2:
                frame[i][halfvres+3*h-k+offset-1 ] = shade*wall[xx][int(yy[k])]
...
def get_sprites(level):
    sheet = pg.image.load('Assets/Sprites/livrosgift'+str(level)+'.png').convert_alpha()
    livros = [[], []]
    swordsheet = pg.image.load('Assets/Sprites/agoravai.png').convert_alpha() 
    sword = []
    for i in range(3):
    ...
    for i in range(4):
        ...
        for j in range(4):
            yy = j*100
            livros[0][i].append(pg.Surface.subsurface(sheet,(xx,yy,131.25,100)))
            livros[1][i].append(pg.Surface.subsurface(sheet,(xx,yy,131.25,100)))

    spsize = np.asarray(livros[0][1][0].get_size())

    sword.append(sword[1]) 
    swordsp = 0 
    
    return livros, spsize, sword, swordsp

def draw_sprites(surf, livros, enemies, spsize, hres, halfvres, ticks, sword, swordsp, rotv):
    ...
    for en in range(len(enemies)):
        ...
        scale = min(enemies[en][3], 2)*spsize*enemies[en][5]/cos2*hres/1000
        vert = halfvres + halfvres*min(enemies[en][3], 2)/cos2 - offset
        hor = hres/2 - hres*np.sin(enemies[en][2])
        if enemies[en][3] > 0.333:
            spsurf = pg.transform.scale(livros[types][cycle][dir2p], scale)
        else:
            spsurf = pg.transform.smoothscale(livros[types][cycle][dir2p], scale)
        surf.blit(spsurf, (hor,vert)-scale/2)

    swordpos = (np.sin(ticks)*10*hres/800,(np.cos(ticks)*10+15)*hres/800)
...
def load_sounds():
    ...
    sounds['step2'] = pg.mixer.Sound('Assets/Sounds/fly.mp3')
    sounds['swoosh'] = pg.mixer.Sound('Assets/Sounds/livrof.mp3')
    ...
    sounds['won'] = pg.mixer.Sound('Assets/Sounds/intro.mp3')
    sounds['music0'] = pg.mixer.Sound('Assets/Sounds/ambiente.mp3')
    sounds['music1'] = pg.mixer.Sound('Assets/Sounds/ambiente.mp3')
...
def load_textures(textures):
    ...
    wall = pg.surfarray.array3d(pg.image.load('Assets/Textures/fundoestante.jpg'))#/255
    bwall = pg.transform.smoothscale(pg.image.load('Assets/Textures/fundoestante.jpg'), (25,25))
